...
{
    public class AppProxy : IHttpHandler, IRequiresSessionState
    {
        ...
        private static string PROXY_REFERER = getConfig().ProxyUrl;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string authenErrorMessage = null;
            if ((!serverUrl.existInProxyConfigFile || !serverUrl.BypassAuthen) &&
                !AuthenUtil.IsValidAuthen(context.Request, context.Session, AuthenUtil.AuthenMode.LOGIN_REQUIRED, out authenErrorMessage))
            {
                context.Response.Clear();
                string userID = "";
                if (AppSession.GetSession("USER_ID", context.Session) != null)
                    userID = AppSession.GetSession("USER_ID", context.Session).ToString();
                if (authenErrorMessage == "DUPLICATE_LOGIN")
                {
                    string errorDetail = string.Format("userID : {0} Duplicate login detected.", userID);
                    log(TraceLevel.Error, errorDetail);
                    sendErrorResponse(context.Response, "", "DUPLICATE_LOGIN", System.Net.HttpStatusCode.OK);
                }
                else
                {
                    ...
                    sendErrorResponse(context.Response, "", "NOT_AUTHORIZED", System.Net.HttpStatusCode.OK);
                }
                return;
            }
            ...
            try
            {
                string csrfToken = getCSRFTokenFromPostBody(postBody, context.Request.ContentType);

                if(string.IsNullOrEmpty(csrfToken))
                    csrfToken = getCSRFTokenFromUri(requestUri);

                //any passthrough request must be check for CSRF by default.
                if ((!serverUrl.existInProxyConfigFile || serverUrl.CheckCSRFToken) && !CSRF.IsCSRFTokenMatch(context.Session, csrfToken))
                {
                ...
                requestUri = removeCSRFTokenFromUri(requestUri);
                requestUri = addTokenToUri(requestUri, token, tokenParamName);

                postBody = removeCSRFTokenFromPostBody(postBody);
...
*/
        ...
        private string getCSRFTokenFromUri(string uri)
        {
            List<string> csrfTokenList = new List<string> { "?CSRF_TOKEN=", "&CSRF_TOKEN=" };

            foreach (string csrfToken in csrfTokenList)
            {
                int csrfTokenStartIndex = uri.IndexOf(csrfToken);
                if (csrfTokenStartIndex >= 0)
                {
                    int csrfTokenEndIndex = uri.IndexOf("&", csrfTokenStartIndex + csrfToken.Length);

                    if (csrfTokenEndIndex == -1)
                        csrfTokenEndIndex = uri.Length;

                    string token = uri.Substring(csrfTokenStartIndex + csrfToken.Length, csrfTokenEndIndex - csrfTokenStartIndex - csrfToken.Length);
                    return token;
                }
            }
        ...
        private string removeCSRFTokenFromUri(string uri)
        {
            int paramsStartIndex = uri.IndexOf("?");

            if (paramsStartIndex >= 0 && paramsStartIndex + 1 < uri.Length)
            {
                string uriWithoutParams = uri.Substring(0, paramsStartIndex);

                string paramsStr = uri.Substring(paramsStartIndex + 1);
                string[] paramList = paramsStr.Split(new char[] { '&' });
                ...
                if (newParamList.Count > 0)
                    return uriWithoutParams + "?" + string.Join("&", newParamList);
                else
        ...
        private string getCSRFTokenFromPostBody(byte[] postBody, string contentType)
        {
            string post = System.Text.Encoding.UTF8.GetString(postBody);
            ...
            try
            {
                ...
                Dictionary<string, object> param = serializer.Deserialize<Dictionary<string, object>>(post);
                if (param.ContainsKey("CSRF_TOKEN"))
                    token = param["CSRF_TOKEN"].ToString();
            }
            catch { };
        ...
        private byte[] removeCSRFTokenFromPostBody(byte[] postBody)
        {
            string post = System.Text.Encoding.UTF8.GetString(postBody);
            ...
            try
            {
                ...
                post = serializer.Serialize(param);
            }
            catch { };
...
*/
        ...
        private static void log(TraceLevel logLevel, string msg)
        {
            ...
            if (logLevel <= ts.Level)
            {
                if(logLevel == TraceLevel.Error)
                    Core.WriteLogError("Proxy.ashx : " + msg);
                else if (logLevel == TraceLevel.Warning)
                    Core.WriteLogInfo("Proxy.ashx, Warning : " + msg);
                else 
                    Core.WriteLogInfo("Proxy.ashx : " + msg);
            }
    ...
    public class ProxyConfig
    {
        ...
        public static DateTime proxyConfigDate = DateTime.Now;
        public static ProxyConfig proxyConfig = null;
        public static ProxyConfig GetCurrentConfig()
        {
            string proxyFilePath = HttpContext.Current.Server.MapPath("~/Configs/AppProxy.config");

            DateTime lastModified = File.GetLastWriteTime(proxyFilePath);

            if (proxyConfig == null || proxyConfigDate != lastModified)
            {
                proxyConfig = LoadProxyConfig(proxyFilePath);
            }
            return proxyConfig;
        }
        ...
        public ServerUrl GetConfigServerUrl(string uri)
        {
            ...
            if (!mustMatch)
            {
                ServerUrl serverUrl = new ServerUrl(uri);
                serverUrl.existInProxyConfigFile = false;
    ...
    public class ServerUrl
    {
        ...
        bool bypassAuthen;
        bool checkCSRFToken;
        public bool existInProxyConfigFile = true;
